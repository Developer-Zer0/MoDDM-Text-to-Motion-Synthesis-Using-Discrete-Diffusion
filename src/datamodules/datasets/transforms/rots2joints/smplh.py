...
class SMPLH(Rots2Joints):
    ...
    def forward(self, smpl_data: dict,
        ...
        if nohands:
            from DetUtil.UtilRotations import to_matrix
            # still axis angle
...
def smplh_to(jointstype, data, trans):
    from src.utils.joint_map_smplh import get_root_idx

    if "mmm" in jointstype:
        from src.utils.joint_map_smplh import smplh2mmm_indexes
        indexes = smplh2mmm_indexes
        data = data[..., indexes, :]

        # make it compatible with mmm
        if jointstype == "mmm":
            from src.utils.joint_map_smplh import smplh_to_mmm_scaling_factor
        ...
        from src.utils.joint_map_smplh import smplh2smplnh_indexes
