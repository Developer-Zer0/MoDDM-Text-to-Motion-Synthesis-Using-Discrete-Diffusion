...
class PoseDecoderGRU(nn.Module):
    def __init__(self, n_poses, pose_dim):
        super().__init__()
    ...
    def forward(self, latent_code):
        feats = latent_code.unsqueeze(1).repeat(1, self.gen_length, 1)
        output, decoder_hidden = self.gru(feats)
        output = output[:, :, :self.hidden_size] + output[:, :, self.hidden_size:]  # sum bidirectional outputs
        output = self.out(output.reshape(-1, output.shape[2]))
        output = output.view(latent_code.shape[0], self.gen_length, -1)
        return output


class PoseEncoderConv(nn.Module):
    def __init__(self, n_poses, dim):
        super().__init__()
        ...
        out_feat = (int((n_poses - 6)/2) - 2)*32

        self.out_net = nn.Sequential(
            # nn.Linear(864, 256),  # for 64 frames
            nn.Linear(out_feat, 256),  # for 34 frames
            nn.BatchNorm1d(256),
            nn.LeakyReLU(True),
            nn.Linear(256, 128),
            nn.BatchNorm1d(128),
...
class ContextEncoder(nn.Module):
    def __init__(self):
        ...
        self.gru = nn.GRU(self.word_latent_dim , hidden_size=256, num_layers=2,
        ...
        self.out = nn.Sequential(
            ...
            nn.Linear(128, self.out_latent_dim)
        )

        self.fc_mu = nn.Linear(self.out_latent_dim, self.out_latent_dim)
        self.fc_logvar = nn.Linear(self.out_latent_dim, self.out_latent_dim)
    ...
    def forward(self, in_text):
        ...
        # for text-to-motion, we only need text tokens
...
class JointEmbeddingNet(nn.Module):
    def __init__(self, n_poses, pose_dim,
                 mode='pose', input_mode='pose', vae=False):
        super().__init__()
        if mode != 'pose': # joint embedding
            self.context_encoder = ContextEncoder()
            self.pose_encoder = PoseEncoderConv(n_poses, pose_dim)
            self.decoder = PoseDecoderGRU(n_poses, pose_dim)
        else: # pose auto-encoder so we use conv for both encoder & decoder.
            self.context_encoder = None
            self.pose_encoder = PoseEncoderConv(n_poses, pose_dim)
            self.decoder = PoseDecoderConv(n_poses, pose_dim)

        self.mode = mode
        self.input_mode = input_mode
        self.vae = vae

    def forward(self, x): #in_text, in_audio, pre_poses, poses):
        in_text = x['text']
        gt_data = x['datastruct']
        poses = gt_data.rfeats
        #poses = torch.flatten(poses, -2, -1)

        input_mode = self.input_mode
        variational_encoding=self.vae
        # context
        if self.context_encoder is not None and in_text is not None:
            context_feat, context_mu, context_logvar = self.context_encoder(in_text)
...
if __name__ == '__main__':
    ...
    feat, pose_mu, pose_logvar = encoder(poses, True)
