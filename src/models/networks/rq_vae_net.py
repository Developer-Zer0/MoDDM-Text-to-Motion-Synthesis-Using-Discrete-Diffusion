...
class RQBottleneck(nn.Module):
    ...
    def __init__(self,
        ...
        self.restart_unused_codes = restart_unused_codes
        self.n_embed = n_embed if isinstance(n_embed, Iterable) else [n_embed for _ in range(n_layers)]
        self.embed_dim = embed_dim
        self.n_layers = n_layers
        self.decay = decay if isinstance(decay, Iterable) else [decay for _ in range(n_layers)]
        ...
        if self.shared_codebook:
            ...
            self.codebooks = nn.ModuleList([codebook0 for _ in range(n_layers)])
        else:
    ...
    def encode(self, x):
        _, codes = self.quantize(torch.transpose(x, 1, 2))
        return codes

    def decode(self, codes):
        return torch.transpose(self.embed_code(codes), 1, 2)
...
if __name__ == '__main__':
    ...
    # code_shape = [8,8,1]
    bottle_neck = RQBottleneck(n_embed=128, embed_dim=64, n_layers=2)
    input = torch.randn(4,45,64)
    quant_list, codes = bottle_neck.quantize(input)
    a = bottle_neck.embed_code_with_depth(codes)
    print(codes.shape)
    print(a.shape)
    print(torch.cumsum(a, -2).shape)
