...
if __name__ == '__main__':
    device = 'cuda'
    input = torch.randn((4, 64, 45)).to(device)
    bottleneck = BottleneckBlock(256, 64, 0.99).to(device)
    zs, xs_quantised, commit_losses, metrics = bottleneck(input)
    print(zs.shape)
    print(xs_quantised.shape)
    print(commit_losses)
    print(metrics)