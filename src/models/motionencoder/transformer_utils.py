    ...
    def forward(
        ...
        for block_idx in range(len(self.blocks)):   
            if self.use_checkpoint == False:
                emb, att_weight = self.blocks[block_idx](emb, cond_emb, t) # B x (Ld+Lt) x D, B x (Ld+Lt) x (Ld+Lt)
            else:
...
class Condition2ImageTransformer(nn.Module):
    def __init__(
        ...
        self.content_emb = instantiate(content_emb_config)
